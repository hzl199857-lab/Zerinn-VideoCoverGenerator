...
function App() {
  ...
  // Initialize default source if T8 Env var exists and no preference
  useEffect(() => {
    // @ts-ignore
    if (!localStorage.getItem('api_source') && process.env.T8_API_KEY) {
      setApiSource('t8');
    }
  }, []);
  ...
  useEffect(() => {
    const checkKey = async () => {
      ...
      if (process.env.T8_API_KEY) {
          if (apiSource === 't8') setHasApiKey(true);
      }
    };
    checkKey();
  }, [apiSource]);
